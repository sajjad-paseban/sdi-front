...
export function setTokenUpdateAwait(value: boolean) {
    tokenUpdateAwait = value
}


export interface MyApiResponse<T1> {
    text: string[],
    status?: number
    result?: T1
}

const authErrorsText: string[] = ['یوزر پیدا نشد', 'یوزر غیر فعال است', 'توکن یوزر عوض شده است', 'توکن یوزر غیر فعال است', 'کاربرلاگین نیست', 'توکن شما منقضی شده است', 'توکن صحیح نیست']
//T1:تایپ دیتای خروجی مورد نظر ما
//T2:تایپ دیتای خروجی از بکند
//T3:تایپ دیتای ارسالی به بکند
export async function sendRequest<T1, T2, T3 = object>(
    url: string,
    method: string,
    validation?: ZodType,
    transform?: (data: T2) => any,
    dataSend?: T3,
    toast?: any): Promise<MyApiResponse<T1>> {


    const myApiResponse: MyApiResponse<T1> = {
        text: [],
        ...
        if (method !== 'get') {
            ...
            if (typeof dataSend !== 'string') {
                if (typeof dataSend === 'object') {
                    (Object.keys(dataSend) as (keyof typeof dataSend)[]).forEach((key, index) => {
    ...
    try {
        const response = await Api.request<T2>({
            url: url,
            method: method,
        ...
        if (axios.isAxiosError(error)) {
            ...
            error.issues.forEach((error) => {
                myApiResponse.text.push(
                    error.message + ' in ' + error.path.toString()
                )
            })
        ...
        if (myApiResponse.text.length > 0) {
            if (toast) {
                myApiResponse.text.forEach((text: string) => {
...
export async function newSendRequest<T1, T2, T3 = object>(
    url: string,
    method: string,
    validation?: ZodType,
    transform?: (data: T2) => any,
    dataSend?: T3,
    itemId?: number | number[] | string | string[],
    toast?: any,
    dataFile = false, setNull = false): Promise<MyApiResponse<T1>> {
...
export default Api as AxiosStatic