<template>
    ...
    <div style="height: 100%;width:100%;display: flex;flex-direction: row;flex-wrap: wrap;align-content: flex-start;justify-content: flex-start;"
        ...
        >
            <div style="width: 50dvw;height:68dvh" v-if="initDataAddModel">
                <GalleryComponent :init-images="initDataAddModel.photos" @updateLinks="updateLinks"
...
export default defineComponent({
    ...
    setup() {
        ...
        onMounted(async () => {
            if (route.params.model && typeof route.params.model === 'string') {
                model_variable.value = route.params.model
                await getInitAddModel({id: 0, model_variable: route.params.model})
        ...
        return {
            activeDialog, dialog, model_variable, previewFormDataValue, printTemp, saveFormData, searchSmallTemps,
        ...
        async saveBtn() {
            ...
            if (this.printTemp) {
                if (!this.printTemp.name) {
                    ...
                    const res = await this.saveFormData(this.printTemp)
                    if (res) {
                        this.$toast.add({
                            group: 'tr',
                            life: 4000,
