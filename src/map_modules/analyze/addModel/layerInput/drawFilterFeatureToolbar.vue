        ...
        <vector_layer :map="map" :z-index="11">
            <template #clusterSource="{vector}">
                <cluster_source :layers="''" ref="drawClusterSourceRef" :map="map" :vector="vector">
                    ...
                    <template v-if="!([2,3,4].includes(activePlugin))&&drawType === 'point'" #select="{vectorSource}">
                        <select_interaction :cluster="true" ref="selectInteractionRef"
...
<script lang="ts">
    ...
    import featureBC from "@/broadcast/featureBroadcast";
    import Feature from 'ol/Feature.js';

    export default defineComponent({
        name: "DrawFilterFeatureToolbarComponent",
        components: {
            feature_toolbar,
            vector_layer,
            select_interaction,
        ...
        setup(props) {
            ...
            const searchLayer = ref<LayerItem>()

            onMounted(() => {
                store.getters.getLayers.map((layer: LayerItem) => {
                    if (layer.id === props.searchLayer_id) {
                        searchLayer.value = layer
                    }
                })
            })
            return {
                ...
                toast, searchLayer, setFeaturesBC, featureBroadcast, route
            }
        },
        data() {
            return {
        ...
        methods: {
            async drawEvent(target: DrawEvent) {
                if (target.type === "drawend" && this.searchLayer) {
                    ...
                    this.drawClusterSourceRef?.drawFeature(features.features)
                    this.drawVectorSourceRef?.removeFeatures([target.feature])
                    this.drawFeatures = target.feature
                    ...
                    if (this.activePlugin === 0) {
                        if (item.id === 1) {
                            ...
                            if(this.drawFeatures)
                                this.setFeaturesBC([this.drawFeatures], String(this.route.name))
                            else {
                                this.toast.add({
                                    group: 'br',
                                    life: 2000,
