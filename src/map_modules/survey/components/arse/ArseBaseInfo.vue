<template>
    <div style="height:100%;width:100%;overflow-y: auto" class="myForm pa-4">

        <v-expansion-panels class="rounded-lg elevation-2 font-weight-bold" multiple>
            ...
            >
                <template v-slot:text>
                    <div style="width:100% " class="rounded-lg ">
                        <v-row class="ma-0 pa-0" v-if="arseAddress">
                            <v-col cols="12" sm="4">
                                <v-text-field
                                        density="compact"
                                        v-model="arseAddress.major_street"
                                        variant="solo"
                                        required
                                >
                            ...
                            <v-col cols="12" sm="4">
                                <v-text-field
                                        ...
                                        v-model="arseAddress.minor_street"
                                        variant="solo"
                                        required
                                >
                                    <template v-slot:prepend-inner>
                            ...
                            <v-col cols="12" sm="4">
                                <v-text-field
                                        ...
                                        variant="solo"
                                        v-model="arseAddress.postal_code"
                                        required
                                >
                                    <template v-slot:prepend-inner>
                                        <v-icon color="dark_blue" icon="mdi-format-color-text"/>
                                    ...
                                    <div v-for="(item,index) in owners"
                                        ...
                                        <div class="bg-white elevation-3 rounded-lg border-t ">
                                            ...
                                            <v-row class="ma-0 pa-0 mt-n1" style="font-size: 11px" align="end">
                                                <v-col cols="6" sm="4" md="3" class="py-2 font-weight-bold">
...
export default defineComponent({
    ...
    setup(props) {
        ...
        const arseAddress = ref<ApartmentAddress>({
            alley: null,
            block_name: null,
        ...
        async function initAddress() {
            ...
            if (!arseAddress.value.major_street && arse.geometry) {
                const data: any = featureToJson(arse.geometry, true)
                const res = await getInitAddress({geoJson: data})
                if (res){
                    arseAddress.value = {...arseAddress.value,...initAddressValue.value}
                }
        ...
        onMounted(async () => {
            ...
            if (props.arse.arse_address) {
                arseAddress.value = {...props.arse.arse_address}

            }
            if (props.arse.arse_owner) {
                owners.value = [...props.arse.arse_owner]
            }
        ...
        return {
            ownerFormRef, correctFloat, setArseOwners,
    ...
    methods: {
        ...
        async nextPageFunc(): ApartmentNextPageFuncRes {
            console.log(this.arseAddress)


            if (this.arse.id) {
                const response = await this.setArseAddress(this.arseAddress, this.arse.id)


                let response2 = null
                if (this.ownerFormRef) {
                    const res = await this.ownerFormRef.validate()
                    if (res.valid && this.arse.id) {
                        console.log(this.owners)
                        response2 = await this.setArseOwners(this.owners, this.arse.id)
                    }
                ...
                if (response && response2) {
                    const res: Arse = {
                        ...this.arse,
                        arse_address: this.arseAddress,
                        arse_owner: this.owners,
                    }
                    return {
                        error: null,
                        result: res
