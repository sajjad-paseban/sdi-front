<template>
    ...
    <div style="height:100%;width:100%;overflow-y: hidden" class=" ">
        ...
        <v-fade-transition>
            <div v-if="initData&&route.name=='allUserExtentsList'" style="height:100%;width:100%;position: relative;"
                 class="pa-1">
                ...
                <v-btn @click="router.push({name:'allUserExtentsListChart'})" v-tooltip.right="'نمودار'"
                       class="elevation-5"
                ...
                <div style="height:100%;width:100%;overflow-y: auto" class="font-weight-bold pl-1">
                    <v-row v-if="showSurveyProcessList.length>0" class="ma-0 pa-0" justify="center">
                        <v-col cols="12" >
                            <div v-for="(item,index) in showSurveyProcessList"
                                 :key="index"
                                 style="width: 100%"
                                 class=" pa-1 my-1">
                                <div class="bg-white elevation-2 rounded-lg "
                                    ...
                                    <v-row class="ma-0 pa-0 myForm pa-2 bg-grey-lighten-4 rounded-lg"
                                        ...
                                        >
                                            <div class="bg-white elevation-2 border-t rounded pa-2"
                                                 style="display:flex;;flex-wrap: wrap;align-content: center;height: 100%">
                                                <p style="color:#0461be">{{ 'درحال انجام :  ' }}</p>
                                                &nbsp;
                                                {{ item.completing }}
                                            </div>
                                        ...
                                        >
                                            <div class="bg-white elevation-2 border-t rounded pa-2"
                                                 style="display:flex;;flex-wrap: wrap;align-content: center;height: 100%">
                                                <p style="color:#0461be">{{ 'بازگشت از کنترلر : ' }}</p>
                                                &nbsp;
                                        ...
                                        >
                                            <div class="bg-white elevation-2 border-t rounded pa-2"
                                                ...
                                                &nbsp;
                                                {{ `${item.checker.user.first_name} ${item.checker.user.last_name}` }}
                                            </div>
                                    ...
                                    <div style="height:40px;width:100%" class="bg-grey-lighten-1 border-t rounded-b-lg">
                                        <v-row class="ma-0 pa-0 pr-3 border-t" justify="start" align="center"
                                               style="height: 100%">
                                            <v-btn
                                                    @click="router.push({name:'surveyProcessMelks',params:{id:item.id,code:item.code,name:user_name}})"
                                                    icon
                    ...
                    <v-fade-transition>
                        <v-row v-if="showSurveyProcessList.length==0"  justify="center" align="center"
                               class="ma-0 pa-0"
                               style="height:100% ">
                            <v-col cols="12"
        ...
        <v-fade-transition>
            <div v-if="initData&&route.name=='allUserExtentsListChart'"
...
export default defineComponent({
    ...
    setup(props) {
        ...
        const {getSurveyProcessList, surveyProcessList,getDate} = survey_panel_functions()
        ...
        async function setShowFeatures() {
            ...
            showSurveyProcessList.value.map((item, index) => {
                let find = false
                features.map((f) => {
        ...
        function setFeaturesColors() {
            ...
            showSurveyProcessList.value.map((process) => {
        ...
        const showSurveyProcessList = ref<SurveyProcessItem[]>([]) as Ref<SurveyProcessItem[]>
        ...
        onMounted(async () => {
            ...
            if (route.params.id && typeof route.params.id === 'string' && route.params.name && typeof route.params.name === 'string') {
                ...
                await getSurveyProcessList(parseInt(route.params.id))
                showSurveyProcessList.value = [...surveyProcessList.value]
                user_name.value = route.params.name
                await setShowFeatures()
                setFeaturesColors()
                store.commit(SurveyMutations.SET_PANEL_TOPIC, `ممیزکار ${route.params.name} : لیست کل محدوده ها`)
            }
        ...
        return {
            ...
            initData,
            myChartRef,
            showSurveyProcessList, surveyProcessList, user_name, setShowFeatures, setFeaturesColors,zoomToFeatures,sub_map
    ...
    methods: {
        ...
        async doFilter(data: DoFilterResult) {
            this.showSurveyProcessList = [...this.surveyProcessList]
            data.keys.map((item) => {
                if (item.value !== null) {
                    if (item.name == 'end_date') {
                        this.showSurveyProcessList = this.showSurveyProcessList.filter((i) => {
                            if (i.end_date) {
            ...
            this.showSurveyProcessList.sort((a, b) => {
    ...
    computed: {
        chartData() {
            return this.showSurveyProcessList.map((item) => {
