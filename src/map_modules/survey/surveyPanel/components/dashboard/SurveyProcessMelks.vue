            ...
            <div style="height:100%;width:100%;position: relative;"
                ...
                <v-menu :location="'top'" offset="5px">
                    <template v-slot:activator="{ props }">
                        ...
                        <v-btn v-bind="props" v-tooltip.right="'افزودن ملک'" class="elevation-5"
                               style="position: absolute;left:20px;bottom:70px;z-index: 1001" icon size="small">
                    ...
                    <v-list>
                        <v-list-item
                                v-for="(item, index) in [{title:'ایجاد ملک جدید',mode:'new'},{title:'انتخاب از نقشه',mode:'select'}]"
                                :key="index"
                                :value="index"
                        >
                            <v-list-item-title @click="addNewMelk(item.mode)">{{ item.title }}</v-list-item-title>
                        </v-list-item>
                    </v-list>
                </v-menu>
                                ...
                                >
                                    <template v-slot:title>
                                        ...
                                        <v-row class="ma-0 pa-0 " style="height:100%">
                                            ...
                                            <v-spacer/>
                                            <p :class="block.status==BlockStatusEnums.Completing?'text-blue':'text-error'"
                                    ...
                                    <template v-slot:text>
                                        ...
                                        <div :style="`height:${expandHeight}px`"
                                             style="min-height:140px;overflow-y: auto " class="mx-n4 mb-n2">
                                                ...
                                                <div class="bg-white elevation-2 rounded-lg "
                                                    ...
                                                    <div style="height:30px;width:100%"
                                                        ...
                                                        <v-row class="ma-0 pa-0 pr-3 border-t" justify="start"
                                                            ...
                                                            <v-btn
                                                                    v-tooltip.top="'حذف عارضه'"
                                                                    @click="deleteArseData(item.code)"
                                                                    icon
                                                                    variant="text"
                                                                    size="x-small"
                                                                    color="primary"
...
export default defineComponent({
    name: "SurveyProcessMelksComponent",
    computed: {
        BlockStatusEnums() {
    ...
    setup(props) {
        ...
        const {getSurveyProcessBlockList, surveyProcessBlockList,getProcessItem,processItemValue} = survey_panel_functions()
        const {getEnumsTable, deleteArse} = survey_functions()
        ...
        async function setShowFeatures(selectedBlock: ShowBlockItem | null = null,i:number=0) {
            ...
            if (processItemValue.value){
              const f: Feature = processItemValue.value.feature.clone()
              f.set('group', 8)
              features.push(f)
            }
        ...
        function calculateHeight(element: any) {
            ...
            expandHeight.value = String((elementHeight - ((blockList_clone.value.length + 1) * 57)))
        }

        async function closeNextLevel() {
            blockDialog.value = false
            await getSurveyProcessBlockList(routeId.value)
            blockList_clone.value = surveyProcessBlockList.value.map((block) => {
        ...
        async function initMelks() {
            await getSurveyProcessBlockList(routeId.value)
            await getProcessItem(routeId.value)
            console.log(processItemValue.value,'sssssssssssssss')
            blockList_clone.value = surveyProcessBlockList.value.map((block) => {
                return {...block, show_block_arse: block.block_arse, searchText: ''}
            })
        ...
        onMounted(async () => {
            ...
            if (route.params.id && typeof route.params.id === 'string' && route.params.code && typeof route.params.code === 'string' && route.params.name && typeof route.params.name === 'string') {
                ...
                store.commit(SurveyMutations.SET_PANEL_TOPIC, `ممیزکار ${route.params.name} : ملک های محدوده ${route.params.code}`)
        ...
        return {
            store,opened_block,
            toast, deleteArse, initMelks, setShowFeatures,
            router, closeNextLevel, blockDialog, routeId,
            route, zoomToFeatures, sub_map, selectedBlock, expandHeight,
            initData, blockList_clone, surveyProcessBlockList, code_location, code_physical_natures
    ...
    methods: {
        ...
        addNewMelk(mode: string) {
            const processId = this.routeId
            let routeData = null
            if (mode == 'new') {
                routeData = this.router.resolve({name: 'Arse', params: {id: processId, code: 0}});
            } else {
                routeData = this.router.resolve({name: 'selectMap', params: {id: processId, code: 0}});
            }

            window.open(routeData.href, '_blank');
        },

        deleteArseData(code: string) {
            this.store.commit('setConfirmDialog', {
                ...
                accept: async () => {
                    ...
                    const res = await this.deleteArse(code)
                    if (res) {
                        await this.initMelks()
