<template>
    ...
    <div dir="ltr">
        ...
        <template v-if="showWorkersDialog">
            <popupWindow :topic="'انتخاب ممیزکار'" :close-btn="true" v-if="winSize!==0" :resize="true" :top="winSize>500?'10px':'10px'"
                ...
                <v-card dir="rtl"
                    ...
                    <div style="height:100%;width:100%;;position: relative;"
                        ...
                        <v-row class="ma-0 pa-0" justify="center" style="height:100%;overflow-y: auto">
                            <v-col cols="12" style="">
                                <div v-for="(item,index) in showSurveyRegionInfoList"
                                     :key="index"
                                     style="width: 100%"
                                     class=" pa-1 my-1">
...
export default defineComponent({
    ...
    setup(props) {
        ...
        const {getSurveyRegionInfo, surveyRegionInfoList, getDate, processItem} = survey_panel_functions()

        const initData = ref(false)
        const winSize = ref(0)
        const showWorkersDialog = ref(true)
        const showSurveyRegionInfoList = ref<SurveyRegionInfo[]>([])
        const selectedWorker = ref<null|number>(null)
        const setSidebarInfo = inject('setSidebarInfo')

        async function setShowFeatures() {
            const features: Feature[] = []
            showSurveyRegionInfoList.value.map((item, index) => {
                item.features.map((i) => {
        ...
        function setFeaturesColors() {
            ...
            showSurveyRegionInfoList.value.map((user) => {
        ...
        onMounted(async () => {
            ...
            showSurveyRegionInfoList.value = [...surveyRegionInfoList.value]
            winSize.value = window.innerWidth
            await setShowFeatures()
            setFeaturesColors()
            if (processItem.value.survey_person){
                selectedWorker.value = processItem.value.survey_person
            }
            setTimeout(async () => {
        ...
        return {
            ...
            router,
            route, getDate,
            initData, showSurveyRegionInfoList, setShowFeatures, setFeaturesColors,setSidebarInfo,
            winSize, showWorkersDialog, surveyRegionInfoList, processItem,selectedWorker
        }
    },
    ...
    methods: {
        selectWorker(item:SurveyRegionInfo){
            ...
            this.setSidebarInfo(0,`${item.user.first_name} ${item.user.last_name}`)
        },

        async doFilter(data: DoFilterResult) {
            this.showSurveyRegionInfoList = [...this.surveyRegionInfoList]
            data.keys.map((item) => {
                if (item.value !== null) {
                    if (item.name == 'userName') {
                        this.showSurveyRegionInfoList = this.showSurveyRegionInfoList.filter((i) => {
            ...
            this.showSurveyRegionInfoList.sort((a, b) => {
    ...
    watch: {
        selectedWorker() {
            if (this.selectedWorker !== null) {
                ...
                this.processItem.survey_person = this.selectedWorker
                console.log(this.processItem)
            }
        }
    }
