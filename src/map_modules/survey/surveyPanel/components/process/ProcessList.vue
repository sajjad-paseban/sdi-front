<template>
    ...
    <div style="height:100%;width:100%;overflow-y: hidden" class=" ">
        ...
        <v-fade-transition>
            <div v-if="initData&&route.name=='surveyProcessList'" style="height:100%;width:100%;position: relative;"
                 class="pa-2 pl-1">
                ...
                <v-btn @click="router.push({name:'surveyProcessListChart'})" v-tooltip.right="'نمودار'"
                       class="elevation-5"
                ...
                <v-btn @click="router.push({name:'selectWorker'})" v-tooltip.right="'ایجاد محدوده ممیزی'"
                       class="elevation-5"
                ...
                <div style="height:100%;width:100%;overflow-y: auto" class="font-weight-bold ">
                    <v-row class="ma-0 pa-0" justify="center">
                        <v-col cols="12" >
                            <div v-for="(item,index) in showProcessList"
                                 :key="index"
                                 style="width: 100%"
                                 class=" pa-1 my-1">
                                    ...
                                    <v-row class="ma-0 pa-0 myForm pa-2 bg-grey-lighten-4 rounded-lg"
                                        ...
                                        >
                                            <div class="bg-white elevation-2 border-t rounded pa-2"
                                                ...
                                                &nbsp;
                                                {{ item.survey_person.user.first_name }}
                                                &nbsp;{{ item.survey_person.user.last_name }}
                                            </div>
                                        ...
                                        >
                                            <div class="bg-white elevation-2 border-t rounded pa-2"
                                                 style="display:flex;;flex-wrap: wrap;align-content: center;height: 100%">
                                                <p style="color:#0461be">{{ 'نام کنترلر : ' }}</p>
                                                &nbsp;
                                                {{ item.checker.user.first_name }} &nbsp;{{
                                                    item.checker.user.last_name
                                                }}
                                            </div>
                                    ...
                                    <div style="height:40px;width:100%" class="bg-grey-lighten-1 border-t rounded-b-lg">
                                        <v-row class="ma-0 pa-0 pr-3 border-t" justify="start" align="center"
                                            ...
                                            <v-btn
                                                    ...
                                                    @click="router.push({name:'processMelks',params:{id:item.id}})"
                                                    icon
                                                    variant="text"
                                                    size="x-small"
                                                    color="primary"
                                            ...
                                            >
                                                <v-img
                                                        @click="router.push({name:'addProcess',params:{id:item.id}})"
                                                        width="25"
                                                        class=""
        ...
        <v-fade-transition>
            <div v-if="initData&&route.name=='surveyProcessListChart'"
...
import {ProcessListItem} from "@/models/SurveyPanel.interface";
import source_functions from "@/components/openLayers/componentFunctions/source";
import Feature from "ol/Feature";


export default defineComponent({
    name: "AllUserExtentsListComponent",
    ...
    setup(props) {
        ...
        const {getProcessList, processList, deleteProcess,getDate} = survey_panel_functions()

        const initData = ref(false)
        const showProcessList = ref<ProcessListItem[]>([]) as Ref<ProcessListItem[]>
        ...
        async function setShowFeatures() {
            ...
            showProcessList.value.map((item, index) => {
        ...
        function setFeaturesColors() {
            ...
            showProcessList.value.map((process) => {
        ...
        async function initProcessList() {
            ...
            showProcessList.value = [...processList.value]
            await setShowFeatures()
            await setFeaturesColors()
            initData.value = true
        }

        onMounted(async () => {
            store.commit(SurveyMutations.SET_MAP_DISABLE, false)
            console.log(store.getters.getMapDisable)
            store.commit(SurveyMutations.SET_PANEL_TOPIC, 'لیست محدوده های ممیزی')
        ...
        return {
            ...
            route,
            initData,
            sub_map, zoomToFeatures, processList, deleteProcess, initProcessList,
            showProcessList, setShowFeatures, setFeaturesColors
        }
    },
    ...
    methods: {
        deleteProcessItem(id: number) {
            this.store.commit('setConfirmDialog', {
                massage: 'آیا حذف مورد تایید است ؟',
                topic: 'اخطار',
                active: true,
                accept: async () => {
                    const res = await this.deleteProcess(id)
        ...
        async doFilter(data: DoFilterResult) {
            this.showProcessList = [...this.processList]
            data.keys.map((item) => {
                if (item.value !== null) {
                    if (item.name == 'checker') {
                        this.showProcessList = this.showProcessList.filter((i) => {
                            return `${i.checker.user.first_name} ${i.checker.user.last_name}`.includes(item.value)
                        })
                    } else if (item.name == 'survey_person') {
                        this.showProcessList = this.showProcessList.filter((i) => {
    ...
    computed: {
        chartData() {
            return this.showProcessList.map((item) => {
