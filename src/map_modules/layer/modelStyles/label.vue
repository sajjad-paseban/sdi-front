                    ...
                    >
                        <v-row class="ma-0 pa-0 mysub">
                            <p style="font-size: 10px; line-height: 20px">
                                در این صفحه میتوانید لیست مدل های لیبل را مشاهده و ویرایش و حذف نمایید.
                            </p>
                        </v-row>
...
<script lang="ts">
    ...
    export default defineComponent({
        name: "LabelModelsComponent",
        components: {EditableTable,LabelLayerStyle},

        setup() {

            const {getListTextStyles,listStyles,deleteTextStyle} = labelStyle()
            ...
            return {
                initData, store,listStyles,deleteTextStyle,getListTextStyles
            }
        },
        data() {
            return {
                tab: "labelModelStyles",
            ...
            actionsClick(value: string, item: any, items: any) {
                if (value == "ویرایش") {
                    ...
                    this.store.commit('setConfirmDialog', {
                        ...
                        active: true,
                        accept: async () => {
                            await this.deleteTextStyle(item.id)
                            await this.getListTextStyles()
                        },
                        reject: () => {
                            console.log('reject')
