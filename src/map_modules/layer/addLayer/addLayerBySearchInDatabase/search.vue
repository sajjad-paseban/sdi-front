            ...
            <v-fade-transition>
                ...
                >
                    ...
                    <v-row class="ma-0 pa-0 pt-1 " justify="end" align="end">
                        <v-btn :disabled="resultSimpleDatabaseLayerSearchList.length===0" @click="saveBtn"
            ...
            <v-row class="ma-0 pa-1" align="center" style="height: 100%">
                ...
                >
                    ...
                    <div class="px-2 " style="" id="table2">
                        <EditableTable
                                v-if="resultSimpleDatabaseLayerSearchList.length>0"
                                ref="editTable2"
                                :headers="headers2"
                                :items="resultSimpleDatabaseLayerSearchList"
    ...
    export default defineComponent({
        ...
        setup(props) {
            const {
                ...
                searchLayerConditions,resultSimpleDatabaseLayerSearch,searchLayerConditionsList, resultSimpleDatabaseLayerSearchList
            } = layer_functions()
            const {getDataBaseTables, dataBaseTables, getTableFields, tableFieldsList} = databases()
            ...
            onMounted(async () => {
                ...
                await getDataBaseTables(props.databaseId)
            ...
            return {
                ...
                editTable,
                resultSimpleDatabaseLayerSearch
                , resultSimpleDatabaseLayerSearchList, dataBaseTables
            }
        },
        data() {
            return {
                layerSelected: null as null | string,
        ...
        methods: {
            async selectLayer(data: string) {
                if (data) {
                    this.store.dispatch(StoreActions.SET_REQUEST_LOADING, {type: 'simple', active: true})
                    await this.getTableFields(data, this.databaseId)
                    let number = 0
                    if (this.tableFieldsList) {
                        this.tableData = this.tableFieldsList.map((item) => {
                            number++
                            return {
            ...
            async showResult() {
                if (this.editTable && this.layerSelected) {
                    ...
                    const data: resultSimpleDatabaseLayerSearchSd = {
                        fields: fields,
                        database_id:this.databaseId,
                        table_name:this.layerSelected
                    }
                    await this.resultSimpleDatabaseLayerSearch(data)
