                    ...
                    <v-form v-if="initData" ref="addAccessForm" class="myForm rounded elevation-2 pa-3"
                        ...
                        <v-row class="ma-0 pa-0">
                            <v-col cols="12" md="4" class="ma-0">
                                <v-text-field density="compact" type="text" variant="solo" :rules="nameRule" required
                                              v-model="access['f_name']">
                            ...
                            <v-col cols="12" md="4" class="ma-0">
                                <v-text-field :rules="nameRule" type="text" density="compact" variant="solo" required
                                              v-model="access.e_name">
                                    <template v-slot:prepend-inner>
                                        <v-icon color="primary" icon="mdi-format-color-text"/>
                                    </template>
...
<script lang="ts">
    ...
    import {default as accessFunctions} from "@/componentFunctions/access_functions";
    ...
    export default defineComponent({
        name: "editAccessComponent",
        components: {PickList},

        setup() {
            const {accesses, apis, getAllApis, getAccesses,editAccess} = accessFunctions()
            ...
            const access = ref<AddAccessSDModel>({
                f_name: '',
                e_name: '',
                parent: null,
                is_active: true,
            ...
            const getEditItemApis = ref<MyApi[]>([])


            async function getEditItem(e_name: string) {
                const response = await Api.getEditAccess<AddAccessSDModel>({data: e_name}, toast)
                if (response.result) {
                    console.log(response.result)
                    let myId = 0
                    getEditItemApis.value = response.result.apis.map((item) => {
                        myId = myId + 1
                        return {
                            text: item,
                            id: myId
                    ...
                    access.value = {...response.result, apis: []}
                }
            ...
            return {
                apisData, accesses, apis, addAccessForm, nameRule, toast, initData, access, getEditItemApis,editAccess
            }
        ...
        watch: {
            apis() {
                this.apisData = [this.apis, this.getEditItemApis]
            },
            ...
            addAccess: async function () {
                if (this.addAccessForm) {
                    ...
                    if (res.valid) {
                        ...
                        this.access.old_e_name=String(this.$route.params.data)


                        const response = await this.editAccess(this.access)
