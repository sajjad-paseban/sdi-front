...
export default defineComponent({
  name: "manageAccessComponent",
  components: {},
  data() {
    return {
      tab: "manageTablesList",

      items: [
        {
          icon: "phone",
          text: "لیست دسترسی ها",
          route: "manageAccessList",
        },
        {
          icon: "phone",
  ...
  methods: {
    ...
    changeTab(){
      if (this.tab !== 'editAccess'){
        this.$router.push({name:this.tab})
      }else {
        this.tab = String(this.$route.name)
        this.$toast.add({group: 'br', life: 4000, severity: 'error', summary: ' خطا', detail: 'دسترسی انتخاب نشده است '});
      }
