...
export default defineComponent({
  name: "manageBaseMapsComponent",
  components: {},
  data() {
    return {
      tab: "manageBaseMapList",

      items: [
        {
          icon: "phone",
          text: "نقشه های زیرین",
          route: "manageBaseMapList",
        },
        {
          icon: "phone",
          text: "افزودن نقشه زیرین",
          route: "addBaseMap",
        },
        {
          icon: "phone",
          text: "ویرایش نقشه زیرین",
          route: "editBaseMap",
        },
  ...
  methods: {
    ...
    changeTab(){
      if (this.tab !== 'editBaseMap'){
        this.$router.push({name:this.tab})
      }else {
        this.tab = String(this.$route.name)
        this.$toast.add({group: 'br', life: 4000, severity: 'error', summary: ' خطا', detail: 'نقشه زیرین انتخاب نشده است '});
      }
    }
  },
});
