                        ...
                        >
                            <v-col class="ma-0 pa-0" cols="12">
                                ...
                                <v-row class="pa-0 ma-0 myFile">
                                    ...
                                    <v-col cols="4" v-if="baseMapImageSrc" align="center">
                                        <v-img
                                                class="rounded-lg"
                                                max-height="90"
                                                max-width="90"
                                ...
                                <v-row class="pa-0 ma-0">
                                    <v-col cols="5" class="pa-0 ma-0">
                                        <v-checkbox v-model="baseMap.default" label="پیشفرض"></v-checkbox>

                                    </v-col>

                                    <v-col cols="5" class="pa-0 ma-0">
                                        <v-checkbox v-model="baseMap.online" label="آنلاین"></v-checkbox>
...
<script lang="ts">
    ...
    export default defineComponent({
        name: "addBaseMapComponent",
        components: {},

        setup(){

            const {addBaseMap} = baseMap()

            const addBaseMapForm = ref<VForm>()
            const {nameRule}= rules()
            return{
                addBaseMapForm,nameRule,addBaseMap
            }
        },
        ...
        data() {
            return {
                ...
                baseMapImageFile:[],
                baseMapImageSrc: null,

                baseMap:{default:false,online:true,code:''} as BaseMapInterface
        ...
        methods: {
            ...
            async addBaseMapAction() {
                if (this.addBaseMapForm) {
                    ...
                    if (res.valid) {
                        console.log(this.baseMapImageFile)
                        await this.addBaseMap({...this.baseMap,image:this.baseMapImageFile[0]})
                    }
                }
