...
export default function wfs_functions() {
    ...
    const getFeaturesByDraw = async (layer, feature, buffer = 0, loading = true, maxFeatures = 1000) => {
        const geo = feature.getGeometry();
        let coords;
        const coordsTextArray = [];
        let cql = '';
        ...
        try {
            if (geo) {
                ...
                coords.map((coordinate) => {
                    coordsTextArray.push(String(coordinate[0]) + ' ' + String(coordinate[1]));
                });
                if (geo instanceof Polygon) {
    ...
    const getFeaturesByFields = async (layer, fields, maxFeatures = 1000) => {
        await store.dispatch(StoreActions.SET_REQUEST_LOADING, { type: 'simple', active: true });
        try {
            const response = await axios({
                method: "get",
    ...
    const makeFilterCql = (dataInputType, operation, value, inputName) => {
    ...
    const makeCql = (field, text, first = false) => {
    ...
    const getShowLayersFeaturesByExtent = async (map, maxFeatures = 1000) => {
        const extent = map.getView().calculateExtent().join(',');
        const storeLayers = store.getters.getLayers;
        await store.dispatch(StoreActions.SET_REQUEST_LOADING, { type: 'simple', active: true });
    ...
    const getLayersByClick = async (layers, feature, type, buffer = 0, maxFeatures = 1000) => {
        const res = [];
        const geo = feature.getGeometry();
        let coords;
        const coordsTextArray = [];
    ...
    const getFeatureByField = async (layer, fieldName, value, maxFeatures = 1) => {
        try {
            const response = await axios({
                method: "get",
                url: layer.service_profile ? (layer.service_profile.wfs_address ? layer.service_profile.wfs_address : layer.service_profile.address.replace('wms', 'wfs')) : store.state.geoUrl + '/wfs',
...
//# sourceMappingURL=wfs_functions.js.map